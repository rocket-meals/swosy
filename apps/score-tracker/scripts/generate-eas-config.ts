...
import { getCustomerConfig } from '../frontend/config';

const TEMPLATE_FILE = path.resolve(__dirname, '../frontend/eas.template.json');
const TARGET_FILE = path.resolve(__dirname, '../frontend/eas.json');
...
function main() {
	...
	const customerConfig = getCustomerConfig() as { appleAppId?: string } & ReturnType<typeof getCustomerConfig>;

	updateAscAppId(template, customerConfig.appleAppId);
