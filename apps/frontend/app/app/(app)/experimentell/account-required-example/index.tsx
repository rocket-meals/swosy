...
const AccountRequiredExample = () => {
	...
	useEffect(() => {
		navigation.setOptions({
			header: () => <CustomStackHeader label="Account Required Example" />,
		});
	}, [navigation]);
	...
	return (
		...
		>
			<View style={styles.content}>
				<Text style={[styles.heading, { color: textColor, textAlign, writingDirection }]}>Account Required - Beispiele</Text>

				{/* ── 1. Default isAccountRequired ─────────────────────────────── */}
				<Text style={[styles.sectionTitle, { color: textColor, textAlign, writingDirection }]}>1. Default isAccountRequired</Text>
				<Text style={[styles.sectionDescription, { color: textColor, textAlign, writingDirection }]}>Standard-Anzeige wenn ein Account erforderlich ist: gestrichelter Rahmen und gedimmtes Schloss-Icon.</Text>
				<SettingsList
				...
				<Text style={[styles.sectionTitle, { color: textColor, textAlign, writingDirection }]}>2. Schloss-Icon rechts</Text>
				<Text style={[styles.sectionDescription, { color: textColor, textAlign, writingDirection }]}>Das Schloss-Icon zeigt deutlich, dass ein Account benötigt wird.</Text>
				<SettingsList
					iconBgColor={primaryColor}
				...
				<Text style={[styles.sectionTitle, { color: textColor, textAlign, writingDirection }]}>3. Gold-Rahmen (Premium)</Text>
				<Text style={[styles.sectionDescription, { color: textColor, textAlign, writingDirection }]}>Ein goldener Rahmen signalisiert Premium-Inhalt.</Text>
				...
				{/* ── 4. Dashed border ──────────────────────────────────────────── */}
				<Text style={[styles.sectionTitle, { color: textColor, textAlign, writingDirection }]}>4. Gestrichelter Rahmen</Text>
				<Text style={[styles.sectionDescription, { color: textColor, textAlign, writingDirection }]}>Ein gestrichelter Rahmen deutet an, dass die Funktion noch "gesperrt" ist.</Text>
				...
				<Text style={[styles.sectionTitle, { color: textColor, textAlign, writingDirection }]}>5. Schimmer-Animation (Blitz-Streifen)</Text>
				...
				<Text style={[styles.sectionTitle, { color: textColor, textAlign, writingDirection }]}>6. Schloss-Badge (Overlay)</Text>
				<Text style={[styles.sectionDescription, { color: textColor, textAlign, writingDirection }]}>Ein kleines Badge-Icon überlagert den Button in der Ecke.</Text>
				...
				<Text style={[styles.sectionTitle, { color: textColor, textAlign, writingDirection }]}>7. Gedimmt (deaktiviert)</Text>
				<Text style={[styles.sectionDescription, { color: textColor, textAlign, writingDirection }]}>Der Button ist sichtbar, aber halbtransparent überlagert und nicht klickbar.</Text>
				...
				<Text style={[styles.sectionTitle, { color: textColor, textAlign, writingDirection }]}>8. Benutzerdefinierter Button (Hintergrund-Schimmer)</Text>
				<Text style={[styles.sectionDescription, { color: textColor, textAlign, writingDirection }]}>Ein komplett eigener Button mit einem Schimmer auf dem Hintergrund.</Text>
				<View style={[styles.shimmerWrapper, { borderWidth: 2, borderColor: '#F5C518' }]}>
					...
					>
						...
						<View style={{ flex: 1 }}>
							<Text style={{ color: '#F5C518', fontFamily: 'Poppins_700Bold', fontSize: 15, textAlign, writingDirection }}>Premium freischalten</Text>
							<Text style={{ color: '#aaa', fontFamily: 'Poppins_400Regular', fontSize: 12, textAlign, writingDirection }}>Account erforderlich</Text>
						</View>
				...
				<Text style={[styles.sectionTitle, { color: textColor, textAlign, writingDirection }]}>9. Pulsierender Rahmen</Text>
				<Text style={[styles.sectionDescription, { color: textColor, textAlign, writingDirection }]}>Der Rahmen pulsiert sanft, um Aufmerksamkeit zu erregen.</Text>
