...
const Index: React.FC<DrawerContentComponentProps> = ({ navigation }) => {
  ...
  const setDefaultPriceGroupForAnonymousUser = () => {
    dispatch({ type: UPDATE_PROFILE, payload: { ...profile, price_group: 'student' } });
  };
  useEffect(() => {
  ...
  const getPriceGroup = (price_group: string) => (price_group ? `price_group_${price_group?.toLocaleLowerCase()}` : '');
