    ...
    <ScrollView style={[styles.container, { backgroundColor: theme.screen.background }]}>
      <View style={styles.field}>
        ...
        {urls.map((url, index) => (
          <TextInput
            key={index}
            style={[styles.input, { color: theme.screen.text, borderColor: theme.screen.icon, marginBottom: 8 }]}
            value={url}
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          router.push({
            pathname: '/rss-feed',
            params: { urls: urls.filter((u) => u.trim()).join(','), switchIntervalInSeconds: interval },
          });
        }}
      >
