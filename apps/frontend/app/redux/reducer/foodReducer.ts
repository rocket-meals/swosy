...
const foodReducer = (state = initialState, actions: any) => {
	switch (actions.type) {
		...
		case CLEAR_POPUP_EVENTS: {
			// Drops all popup events including their isOpen flags - the flag doubles as
			// the persisted "already seen/dismissed" marker, so clearing it here is what
			// makes dismissed popup events show up again for the next login.
			return {
				...state,
				popupEvents: [],
			};
