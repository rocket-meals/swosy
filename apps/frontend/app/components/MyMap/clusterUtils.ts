...
export const DEFAULT_CLUSTER_PIXEL_RADIUS = CLUSTER_PIXEL_RADIUS;
...
/**
 * Static colour mapping for cluster badges based on fixed size thresholds.
 * Colours progress from green (small) to purple (large).
 */
function getClusterColor(count: number): string {
	if (count >= 100) return '#A855F7'; // purple
	if (count >= 50) return '#EF4444';  // red
	if (count >= 20) return '#FB923C';  // orange
	if (count >= 10) return '#FACC15';  // yellow
	return '#34D399';                   // green (covers counts < 10)
}
...
function createClusterSvg(count: number): string {
	...
	const fill = getClusterColor(count);
...
function getGridCell(lat: number, lng: number, zoom: number, pixelRadius: number): string {
	// Leaflet uses 256-pixel tiles. At zoom level z the world is 256*2^z pixels wide (360°).
	// cellDeg is the geographic width/height of one grid cell that corresponds to pixelRadius pixels.
	const cellDeg = (pixelRadius / 256) * (360 / Math.pow(2, zoom));
	const col = Math.floor(lng / cellDeg);
	const row = Math.floor(lat / cellDeg);
	return `${col}:${row}`;
}

export function clusterMarkers(markers: MapMarker[], zoom: number, pixelRadius: number = CLUSTER_PIXEL_RADIUS): MapMarker[] {
	...
	for (const marker of markers) {
		const key = getGridCell(marker.position.lat, marker.position.lng, safeZoom, pixelRadius);
