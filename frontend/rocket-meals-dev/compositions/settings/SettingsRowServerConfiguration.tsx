...
import {DrawerConfigPosition} from "@/states/DrawerSyncConfig";
...
export const SettingsRowServerConfiguration: FunctionComponent<AppState> = ({...props}) => {
    ...
    let availableOptionKeys = Object.keys(optionKeyToName) as ServerConfigurationsOptions[]
    // check which server is selected by the url so iterate over all optionKeyToUrl and check if the url is the same
    for(let key in optionKeyToUrl){
    ...
    for(let key of availableOptionKeys){
        ...
        let nextUrl = optionKeyToUrl[key as ServerConfigurationsOptions]
        let active = nextUrl === activeUrl
