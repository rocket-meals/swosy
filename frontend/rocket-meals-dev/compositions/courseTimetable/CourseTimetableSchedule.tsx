...
export const CourseTimetableSchedule: FunctionComponent<AppState> = (props) => {
	...
	const amountOfDays = props?.amountOfDays || 7;

	const defaultAmountOfDaysToShowOnScreen = 7;
	const amountOfDaysToShowOnScreen = props?.amountOfDaysToShowOnScreen || defaultAmountOfDaysToShowOnScreen;
	...
	function getEvent(id: string, start: string, end: string, weekday: Weekday) {
		const useDate = DateHelper.getDefaultWeekdayDate(weekday)
		const startDate = new Date(useDate);
	...
	function parseTimetableEventsToList(timetableEvents: CourseTimetableDictType) {
		const events = [];
		const keys = Object.keys(timetableEvents)
		for (let i=0; i<keys.length; i++) {
			const key = keys[i];
			const event = timetableEvents[key];
			if (event) {
				const parsedEvent = getEvent(event.id, event.start, event.end, event.weekday);
	...
	function renderHeader({date, start, end}) {
		...
		const weekday = DateHelper.getWeekdayNameByDate(startDate, locale);
		return (
			<View>
				<Text>{weekday}</Text>
			</View>
	...
	function renderTime(fromHour, toHour) {
		const hour = fromHour < 10 ? '0'+fromHour : fromHour;
		return (
