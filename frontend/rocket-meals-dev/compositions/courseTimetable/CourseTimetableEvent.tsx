...
export const CourseTimetableEvent: FunctionComponent<AppState> = (props) => {
	...
	function renderTextEditRow(field: string, description: string){
		...
		return (
			<SettingsRowTextEdit leftIcon={IconNames.attribute_text_icon} accessibilityLabel={
				translationEvent+": "+description
			} value={value} labelLeft={description} onSave={(newValue, hide) => {
	...
	function renderTimeEditRow(field: string, description: string, icon: string){
		...
		return (
			<SettingsRowTimeEdit leftIcon={icon} accessibilityLabel={translationEvent+": "+description} value={value} labelLeft={description} onSave={(newValue, hide) => {
				usedEvent[field] = newValue;
				handleEdit(usedEvent, hide);
			}} />
		)
	...
	if(!newEvent){
		let label = translationDelete;
		let accessibilityLabel = translationEvent+": "+translationDelete;
		...
		let label = translationSave;
		let accessibilityLabel = translationEvent+": "+translationSave;
