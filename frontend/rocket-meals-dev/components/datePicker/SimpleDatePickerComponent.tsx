...
export const SimpleDatePickerComponent: FunctionComponent<SimpleDatePickerProps> = (props) => {
	...
	function renderWeekdayRowForMonth() {
		const output = [];
		const weekDayShortChars = DateHelper.getWeekdayNamesFirstLetters(locale, weekStartsAtDay);
		const weekDayNamesLong = DateHelper.getWeekdayNames(locale, weekStartsAtDay);

		const backgroundColor = props?.weekdayBackgroundColor || defaultBackgroundColor;
		const textColor = props?.weekdayTextColor || defaultSelectedColor

		for (let i = 0; i < amountDaysPerWeek; i++) {
			const weekDayName = weekDayNamesLong[i];
			const weekDayShortChar = weekDayShortChars[i];
	...
	function renderDefaultDayInMonth(index: number, dateToRender: Date | null, referenceDate: Date | null) {
		...
		const monthOfDateToRender = dateToRender.getMonth();
		const monthOfReferenceDate = referenceDate.getMonth();
		...
		const isSelected = dateToRender.toISOString()===referenceDate.toISOString()
		if (isSelected) {
	...
	function renderDaysInMonth(date: Date, firstDayOfWeek: Weekday): (Date | null)[]
	{
		...
		const diffToStartWithFirstDayOfWeek = DateHelper.getAmountDaysFromLastMonthForWeekstart(date, firstDayOfWeek)

		const output: (Date | null)[] = [];
		...
		// current month
		const amountDaysInMonth = DateHelper.getAmountDaysInMonth(date);
		...
		const maxItemsInCalendarDisplay = amountDaysPerWeek*amountCalendarDateRows
	...
	function renderAllDaysInMonth(date: Date) {
		const output: (Date | null)[] = renderDaysInMonth(date, weekStartsAtDay);

        type DataItem = { key: string; data: Date | null; }
        const outputWithKeys: DataItem[] = []
		...
		return (
			...
			>
				<MyPreviousNextButton translation={translatedWordYear}
					forward={forward}
					onPress={() => {
						const nextDate = new Date(shownDate);
	...
	function renderSwitchMonthRow(shownDate: Date) {
		const monthName = DateHelper.getMonthName(shownDate, locale)
